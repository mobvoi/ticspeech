...
#if defined(ANDROID) && __ANDROID_API__ >= 21 && defined(__linux__) && defined(FALLOC_FL_PUNCH_HOLE) && defined(FALLOC_FL_KEEP_SIZE)
...
#elif defined(ANDROID) && __ANDROID_API__ < 21 && defined(__linux__) && defined(FALLOC_FL_PUNCH_HOLE) && defined(FALLOC_FL_KEEP_SIZE)
  UTIL_THROW(UnsupportedOSException, "fallocate hole punching requires Android and __ANDROID_API__ >= 21");
#elif defined(__linux__) && defined(FALLOC_FL_PUNCH_HOLE) && defined(FALLOC_FL_KEEP_SIZE)
  UTIL_THROW_IF_ARG(-1 == fallocate(fd, FALLOC_FL_PUNCH_HOLE | FALLOC_FL_KEEP_SIZE, offset, size), FDException, (fd), "in punching a hole at " << offset << " for " << size << " bytes.");
#else
...
#if defined(_WIN32) || defined(_WIN64) || defined(__APPLE__) || defined(ANDROID) || defined(__MINGW32__)
...
void WriteOrThrow(std::FILE *to, const void *data, std::size_t size) {
  if (!size) return;
...
#if defined(__QNXNTO__)
  std::FILE *ret = std::fdopen(file.get(), "rb");
#else
